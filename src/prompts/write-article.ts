 ...
 *
 * **"설명은 근거 규율에서 예외" 라는 문구를 넣었다가 되돌렸다.** 근거 규율이
 * 설명 문장까지 지운다는 가설이었는데, 측정은 반대였다 — 문장이 오히려 평균 1.4단어
 * 더 길어지고 1회 통과가 9 → 6 으로 줄었으며 성공 편당 비용이 $0.19 → $0.22 로 올랐다.
 * 근거는 D-47. 문장 길이 문제는 이 절과 무관하니 따로 다룬다.
 */
